...
(
   ...
   FROM
      ...
      INNER JOIN
         tab AS t 
         ON s.visit_date = t.last_visit 
         AND s.visitor_id = t.visitor_id 
      LEFT JOIN
         leads AS l 
         ON s.visit_date <= l.created_at 
         AND s.visitor_id = l.visitor_id 
)
,
ads AS 
(
...
(
   ...
   WHERE
      lp.status_id = 142) AS purchases_count,
...
(
   SELECT
      ...
      SELECT
         COUNT(lead_id) 
      FROM
         leads) * 100.00 / COUNT(visitor_id), 2) AS lead_cr 
      FROM
         sessions 
)
,
...
(
   SELECT
      ROUND(COUNT(lead_id) FILTER(
   WHERE
      status_id = 142) * 100.00 / COUNT(lead_id), 2) AS purchase_cr 
   FROM
      leads 
)
,
   ...
   SELECT
      ...
      CASE
         ...
         THEN
            lead_id 
      END
) AS purchases_count, ROUND(100.0 * COUNT(DISTINCT lead_id) / NULLIF(COUNT(DISTINCT visitor_id), 0), 2) AS cr_click_to_lead, ROUND(100.0 * COUNT(DISTINCT 
      CASE
         WHEN
            status_id = 142 
         THEN
            lead_id 
      END
) / NULLIF(COUNT(DISTINCT lead_id), 0), 2) AS cr_lead_to_purchase, ROUND(100.0 * COUNT(DISTINCT 
      CASE
         WHEN
            status_id = 142 
         THEN
            lead_id 
      END
) / NULLIF(COUNT(DISTINCT visitor_id), 0), 2) AS cr_click_to_purchase 
   FROM
      last_paid 
   GROUP BY
      utm_source, utm_medium, utm_campaign 
   ORDER BY
      cr_click_to_lead DESC 
)
...
(
   SELECT
      ...
      SUM(leads) OVER (
   ORDER BY
      lead_date) AS cum_lead,
      SUM(purchases) OVER (
   ORDER BY
      lead_date) AS cum_purchase 
   FROM
      (
         SELECT
...
(
   SELECT
      ...
      cum_lead,
      cum_purchase,
      ROUND(100.0 * cum_purchase / (
      SELECT
         SUM(purchases) 
      FROM
         lead_data), 2) AS purchase_percent 
      FROM
         lead_data 
)
,
...
(
   SELECT
      CORR(spent, visit_cnt) AS correlation 
   FROM
      ads_spent AS ad 
      LEFT JOIN
         organic_traffic AS ot 
         ON ad.campaign_date = ot.visit_date 
)
