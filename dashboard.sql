WITH tab AS (
  ...
  SELECT 'paid_users' AS metric_name, COUNT(lead_id) FILTER(WHERE status_id = 142) AS cnt FROM leads
    ...
    SELECT 
        ...
        spent,
        visit_cnt,
        RANK() OVER (ORDER BY spent) AS rank_x,
        RANK() OVER (ORDER BY visit_cnt) AS rank_y
    FROM ads_spent AS ad
    LEFT JOIN organic_traffic AS ot ON ad.campaign_date = ot.visit_date
), differences AS (
    SELECT 
        ...
        rank_y,
        (rank_x - rank_y) AS d,
        POWER(rank_x - rank_y, 2) AS d_squared
    FROM ranked_data
), sum_d_squared AS (
    SELECT SUM(d_squared) AS sum_d2, COUNT(*) AS n FROM differences
)